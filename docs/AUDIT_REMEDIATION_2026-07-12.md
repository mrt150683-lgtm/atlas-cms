...
| AR-013 | complete | Close contract gaps discovered by the final Sentinel run. | Notes can now be edited through the existing update API, the trust-loop alignment action uses the live `cms align --scan` syntax, and README usage covers the live align, scope, and bundle groups; UI tests and Sentinel contract checks verify the surfaces. |
