...
| **anchor_drift** (Anchor Integrity) | Deterministically compares each `@memory:summary` symbol and `@memory:connects` link with current source and graph evidence. Summary mismatches are medium; unsupported static connections are low because runtime/conceptual coupling may exist beyond the graph. |
